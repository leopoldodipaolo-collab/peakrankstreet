...
window.loadMapAndData = function(lat, lon, zoom = 13, city = null) {
    ...
    // Rimuovi eventuali marker della ricerca città
    ...
    // Mostra stati di caricamento nelle liste
        ...
        .then(data => {
            ...
            if (data.routes && Array.isArray(data.routes)) {
                data.routes.forEach(r => {
                    if (!r.coordinates || !r.coordinates.geometry || !r.coordinates.geometry.coordinates[0]) return;
    ...
    // ✅ Aggiorna percorsi classici solo per la città corrente
    if (city) {
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(cityName)}&format=json&limit=1`)
            ...
            .then(data => {
                if (data && data.length > 0) {
                    ...
                    const cityDisplayName = data[0].display_name.split(',')[0].trim();

                    // Aggiorna la mappa e i marker
                    loadMapAndData(lat, lon, 13, cityDisplayName);

                    // Aggiorna i percorsi classici per la città cercata
                    loadClassicRoutes(cityDisplayName);
    ...
    // Event listener per ricerca con invio
    if (citySearchInput) {
    ...
    // Event listener per pulsante ricerca
    if (citySearchButton) {
        citySearchButton.addEventListener('click', () => searchCity(citySearchInput?.value || ''));
    }

    // Caricamento iniziale della città
    const initialCity = userInitialCity ? userInitialCity.split(',')[0].trim() : "Italia";
